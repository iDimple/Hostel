            ...
            <div class="normal-div">我的会员卡号：
                ...
                <c:choose>
                    <c:when test="${info.state == 'invalid'}">
                        ...
                        <button class="button" onclick="window.location.href='http://localhost:8080/Hostel/validate'">立即激活</button>
                    </c:when>
                    ...
                    <c:when test="${info.state == 'pause'}">
                        ...
                        <button class="button" onclick="window.location.href='http://localhost:8080/Hostel/user/recharge'">恢复使用</button>
                        <button class="button" onclick="stop()">停止账户</button>
                    </c:when>
            ...
            <div class="normal-div">我的账户余额：
                <span class="number">${info.level.balance}
                    <c:if test="${info.state != 'invalid'}">
                        <button class="button" onclick="window.location.href='http://localhost:8080/Hostel/user/recharge'">立即充值</button>
                    </c:if>
                </span>
            </div>
            <div class="normal-div">我的积分剩余：
                <span class="number">${info.level.point}
                    <button class="button" onclick="window.location.href='http://localhost:8080/Hostel/user/point'">积分兑换</button>
                </span>
            </div>
...
<script>
    ...
    function stop() {
        ...
        if (flag == true) {
            $.ajax({
                type: "POST",
                url: "http://localhost:8080/Hostel/user/stop",
                success: function (data) {
                    if (data["success"] == false) {
                        ...
                        setTimeout(function () {
                            window.location.href = "http://localhost:8080/Hostel/dashboard";
                        }, 1000);
                    }
                },
                error: function () {
